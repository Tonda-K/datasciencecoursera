makeCacheMatrix <- function(m = matrix()) {
      ...
      getInv <- function() inv.m    ## if cacheSolve is called than assign value of inverse matrix ##
...
cacheSolve <- function(m, ...) {
      ...
      new_matrix <- m$get ## if there were changes in previous matrix via m$set 
      inv.m <- solve(new_matrix, ...)  # than calculates inverse matrix again ## 
